...
/*
             ...
             impelentation is on line number :-115

             for maximum height go to line number 131
             for largest node go to line number 150
             total number of greater node in the tree line number 171
...
// calculate the total greater number  in the tree

int greaterNode(TreeNode<int> *root, int x)
{
    int count = 0;
    if (root->data > x)
    {
    ...
    for (auto child : root->children)
    {
        count += greaterNode(child, x); // recursive function call
    }
    return count;
}

// for max sum
int maximumGreaterSum(TreeNode<int> *root)
{
    int sum = root->data;
    for (auto child : root->children)
    {
    ...
    for (auto child : root->children)
    {
        int currentMaxsum = maximumGreaterSum(child);
...
int main()
{
    ...
    cout << "Enter the number to find the total greater number" << endl;
    int x;
    cin >> x;
    cout << "so the maxmum sum of the tree is  " << greaterNode(root, x) << endl;
    cout << "the maximum sum of the tree is" << endl;
    cout << maximumGreaterSum(root);
}
